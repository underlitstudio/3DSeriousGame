...
public class AdsManagerEditor : Editor
{
	...
	{
		EditorGUILayout.HelpBox("1. Make sure you have JDK, Android SDK installed on your PC (By Unity Installer or Manually) and linked in your Unity Editor from (Edit > Preferences > External Tools). (Optional) as well as install/update the Extras , Google Play Services from the Android SDK Manager in Android Studio", MessageType.None);

		EditorGUILayout.Separator();
		EditorGUILayout.HelpBox("(Optional) You have the choice to download JDK/Android Studio", MessageType.None);
		...
		EditorGUILayout.HelpBox("2. Download GoogleMobileAd unity package, and then import it to your unity project", MessageType.None);
		...
		EditorGUILayout.HelpBox("6. (Optional) To test the Ads on your mobile device (Test Mode) : \n\n" +
			...
			"- Check the logcat output e.g. from 'Android Studio Logcat layout or Unity Logcat' for a message which shows your device ID\n" +
